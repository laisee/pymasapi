...
try:
    from src.helper import (apply_format, apply_format_level, get_datetime,
                            get_response, get_timestamp, guard)
except (ImportError, ModuleNotFoundError) as err:
    print(err)
    try:
        from pymasapi.src.helper import (apply_format, apply_format_level,
                                         get_datetime, get_response,
                                         get_timestamp, guard)
    except (ImportError, ModuleNotFoundError) as err:
        print(err)
...
class Test_Helper:
    def test_apply_format(cls):
        ...
        """
        assert (
            apply_format(88.1234567) == "88.12346"
        ), "method apply_format() result is not correct: %s " % apply_format(88.1234567)
    ...
    def test_apply_format_level(cls):
        ...
        :return: None
        """
        assert apply_format_level(88.1234567) == "88.12", (
            "method apply_format_level() result is not correct: %s "
            % apply_format_level(88.12345)
        )
    ...
    def test_get_timestamp(cls):
        ...
        """
        assert (
            get_timestamp() is not None
        ), "method get_timestamp() response is not valid"
    ...
    def test_guard_success(cls):
        ...
        """
        assert (
            guard("resourceid", "https://dev.null/?and1") is None
        ), "method guard() response is not valid"
